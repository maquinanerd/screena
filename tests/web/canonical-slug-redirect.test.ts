...
describe("paginas publicas ligam a decisao ao permanentRedirect", () => {
  ...
  it.each(PAGES)("pagina de %s chama canonicalRedirectPath com o indexPath da propria rota", (
    _kind,
    source,
    indexConst,
  ) => {
    expect(source).toContain(
      `canonicalRedirectPath(${indexConst}, slug, data.canonicalSlug)`,
    );
    expect(source).toContain("if (redirectPath !== null) permanentRedirect(redirectPath);");
  });

  it.each(PAGES)("pagina de %s redireciona depois do notFound e antes de renderizar", (
    _kind,
    source,
  ) => {
    const notFoundAt = source.indexOf("if (data === null) notFound();");
    const redirectAt = source.indexOf("permanentRedirect(redirectPath)");
    const renderAt = source.indexOf("return (");
...
describe("constantes de rota usadas pelas paginas", () => {
  it("PESSOAS_INDEX_PATH da pagina de pessoa e o mesmo PEOPLE_INDEX_PATH das rotas", () => {
    expect(PERSON_PAGE).toContain('const PESSOAS_INDEX_PATH = "/pt/pessoas/"');
    expect(PEOPLE_INDEX_PATH).toBe("/pt/pessoas/");
  });

  it("SERIES_INDEX_PATH da pagina de serie e o mesmo das rotas", () => {
    expect(SERIES_PAGE).toContain('const SERIES_INDEX_PATH = "/pt/series/"');
