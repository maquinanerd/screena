 ...
 *
 * ESCOPO (Fase 6B/6C). E protecao operacional MINIMA, nao autenticacao completa:
 ...
 *
 * Comportamento (fail closed por ambiente — Fase 6C):
 *   - Dev/local, protecao NAO exigida            -> segue (acesso aberto, explicito).
 *   - Production-like (producao/preview)          -> protecao SEMPRE exigida, mesmo
 *     sem `ADMIN_PROTECTION_ENABLED` ou com ela `"false"` (nunca sobe aberto).
 *   - Protecao exigida, sem usuario/senha em ENV  -> 401 (nega por seguranca).
 *   - Protecao exigida, Basic Auth ausente/invalido -> 401 + desafio.
 *   - Protecao exigida, Basic Auth valido         -> segue.
 *
 * As cinco ENV sao lidas como referencias literais a `process.env.*` para que o
 * Next as injete no Edge runtime (`NODE_ENV`/`VERCEL_ENV` detectam production-like;
 * as tres de auth controlam o portao). A senha nunca aparece no HTML nem em log.
 */
 ...
 * `/content-blocks`, `/health`, `/security`, ...), exceto os assets internos do
 * Next e o favicon — que nao expoem dado editorial e cujo bloqueio so quebraria
 * o app.
 */
export const config = {
