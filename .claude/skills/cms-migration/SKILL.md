...
> ## ⚠️ Restricoes da Cinerie
>
> Conteudo VENDORIZADO de `payloadcms/skills` (commit `832d5bc`, 2026-07-07),
> alinhado a Payload `^3.0.0`. Rodamos **3.86.0** — compativel.
>
> **Relevancia baixa neste repo.** Nao ha migracao de CMS em curso: o editorial
> entra por contrato (`editorial-draft-v1` do MNScr) ou pela redacao humana no
> proprio Payload, nunca por importacao de WordPress/Contentful. Trate esta
> skill como referencia de modelagem de campo, nao como plano de trabalho.
> Lembre que `CLAUDE.md` §10 mantem WordPress fora do core.
>
> Uma orientacao upstream foi **removida**: a opcao de "keep raw HTML" para
> corpo de materia. O contrato recusa markup HTML por regex
> (`packages/editorial-contracts/src/common.ts`) e o corpo e
> `type: 'blocks'` com `editorialBlocks` — nunca `richText`, nunca HTML cru.
> Os exemplos abaixo que usam `type: 'richText'` sao upstream e ilustrativos;
> nao os aplique a `articles.body`.
...
4. **Rich text** - HTML conversion needs
   <!-- CINERIE: upstream oferecia "or keep raw" (HTML cru). REMOVIDO — o
   contrato recusa markup HTML; converta para `editorialBlocks`. -->
...
| Rich text ambiguity | Clarify: Lexical editor or Slate. (CINERIE: "store raw HTML" REMOVIDO — o contrato recusa HTML; use `editorialBlocks`.) |
