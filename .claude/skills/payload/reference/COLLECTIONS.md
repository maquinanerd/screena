...
export const Posts: CollectionConfig = {
  ...
  admin: {
    useAsTitle: 'title',
    // _status comes from versions.drafts below
  ...
  fields: [
    ...
    slugField(), // unique + indexed, sidebar position
  ],
...
> Enabling `versions: { drafts: true }` injects a managed `_status` field
...
>
> **CINERIE:** o upstream continuava aqui com "Don't add a custom `status`
> select for publish state". **REMOVIDO.** Em `articles`, `workflowStatus` (12
> estados) e a fonte da verdade do fluxo editorial e coexiste com `_status` (2)
> de proposito — `apps/cms/src/workflow.ts`. O mesmo vale para a slug: o
> `slugField()` do core serve a collections internas, mas `articles.slug` sai de
> `canonicalizeSlug` + `RESERVED_SLUGS`. Ver as restricoes no topo de
> [../SKILL.md](../SKILL.md).
