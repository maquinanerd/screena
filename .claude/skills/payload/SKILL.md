...
> ## ⚠️ Restricoes da Cinerie (leia ANTES de aplicar qualquer default daqui)
>
> Este arquivo e conteudo VENDORIZADO de `payloadcms/skills` (commit `832d5bc`,
> 2026-07-07), alinhado a Payload `^3.0.0`. Rodamos **3.86.0** — compativel.
>
> Tres defaults upstream foram **removidos** deste arquivo por contradizerem o
> contrato de publicacao. Nao os reintroduza a partir da memoria nem da doc
> oficial:
>
> 1. **`workflowStatus` NAO e redundante com `_status`.** O upstream manda nao
>    criar campo de status proprio. Aqui o `_status` do Payload (2 valores) nao
>    e o estado editorial: `workflowStatus` tem **12** e e a fonte da verdade —
>    `apps/cms/src/workflow.ts`, `apps/cms/src/collections.ts` (aba Fluxo).
>    Dele dependem o gate de publicacao, a quota e a separacao de atores do
>    [ADR 0017](../../../docs/adr/0017-automation-publisher-actor.md).
> 2. **Nao troque a slug de `articles` por `slugField()` do core.** O valor sai
>    de `canonicalizeSlug` (`apps/cms/src/canonical-slug.ts`) — a MESMA funcao
>    da autopublicacao — com blocklist de `RESERVED_SLUGS`. O helper do core nao
>    aplica nenhum dos dois; usa-lo deixaria passar slug reservada (`api`,
>    `admin`, `pt`) e dessincronizaria publicacao manual e automatica.
> 3. **O corpo de materia e `type: 'blocks'`, nunca `type: 'richText'`.** Os
>    exemplos upstream abaixo usam `richText` livremente — sao ilustrativos, nao
>    aplicaveis a `articles.body`. O contrato recusa markup HTML por regex em
>    `packages/editorial-contracts/src/common.ts`. Formatacao existe (marcacoes
>    por offset ao lado do texto limpo); o que o contrato proibe e **HTML**, nao
>    formatacao. `lexicalEditor()` fica configurado em `payload.config.ts`
>    porque o Payload exige um editor — nao porque o corpo o use.
>
> Componentes customizados de admin (`admin.components`, `useField` de
> `@payloadcms/ui`) **nao** conflitam: e exatamente o padrao ja usado em
> `apps/cms/src/admin/`. Pode seguir o upstream ai.
...
- **Enable drafts/versions by default:** `versions: { drafts: true }`. This is the
  ...
  `_status` field (`draft` / `published` / `changed`). Only skip versions for
  collections that have no publish/draft lifecycle (e.g. internal join tables,
  settings).
  <!-- CINERIE: o upstream seguia aqui com "don't add your own `status` field,
  it's redundant". REMOVIDO — ver restricao 1 no topo. `workflowStatus` (12
  estados) coexiste com `_status` (2) de proposito. -->
  <!-- CINERIE: o upstream tinha aqui um bullet mandando usar `slugField()` do
  core em vez de slug propria. REMOVIDO — ver restricao 2 no topo. O helper
  existe mesmo em 3.86.0 e serve para collections novas SEM identidade publica;
  o proibido e troca-lo pela slug de `articles`. -->
- **`slugField()` existe no core (3.86.0)** e serve bem para collections
  internas. **Nao** o use em `articles`: aquela slug e identidade publica e sai
  de `canonicalizeSlug` + `RESERVED_SLUGS`.
...
export const Posts: CollectionConfig = {
  ...
  admin: {
    useAsTitle: 'title',
    // _status (from versions.drafts) shows the draft/published state
    // CINERIE: nao leia isso como "logo, nao crie campo de status" — ver restricao 1 no topo
...
// Select (for genuine taxonomy)
// CINERIE: `contentType` e `workflowStatus` sao selects legitimos — ver restricao 1 no topo
...
6. **Draft status** — `_status` field is auto-injected when drafts are enabled
   (CINERIE: `_status` != `workflowStatus`; os dois coexistem — restricao 1 no topo)
...
- Enable `versions: { drafts: true }` by default on content collections
  <!-- CINERIE: upstream dizia "rely on the auto-injected `_status` rather than
  adding a custom `status` field" e "use `slugField()` instead of hand-rolling".
  AMBOS REMOVIDOS — ver restricoes 1 e 2 no topo. -->
