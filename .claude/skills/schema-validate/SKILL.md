...
description: >-
  Valida o JSON-LD (Schema.org) de uma pagina da Screena por tipo de entidade:
...
Esta skill valida o **structured data (JSON-LD)** de uma pagina da Screena
...
- `Review` apenas quando a Screena tem **review propria** sobre a entidade.
- `author` e a Screena (ou o autor editorial real), nunca uma fonte de terceiro.
...
- **`AggregateRating` fingindo nota propria.** A Screena nao publica nota
