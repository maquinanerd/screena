...
| TMDB (trailers) | catálogo | `tmdb` | `video` | official | global | sim | sim | n/a | sim | sim | **não** | **não** | **não** | `cinerie-source-auth/tmdb-video/2026-08-v1` | 13/08/2026. Autoriza o **metadado** do vídeo (site/key/tipo/idioma), não o arquivo: o vídeo não é baixado, reproduzido por nós nem rehospedado. O player é do YouTube, sob os termos do Google, e carrega só após clique explícito (política de privacidade, item 6.1). Registrar esta licença **não** liga `tmdb_videos.display_allowed` — a promoção é passo próprio, como em ratings e streaming |
