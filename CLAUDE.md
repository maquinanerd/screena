...
- [`docs/operations/omdb-coverage-and-quota.md`](docs/operations/omdb-coverage-and-quota.md) — **por que 99,13% dos filmes nao tinham nota externa com o worker rodando verde**. A janela de frescor de 168h era aplicada a AUSENCIA: um titulo com zero notas nao esta defasado, ele nunca foi perguntado. Tem tambem: por que o teto de 200 era o relogio e nao a cota (24,5x); a divisao 85/15 entre cobertura e atualizacao, com a conta que a fixa; por que filme/serie e 59/41 e nao metade a metade; a ordem editorial e como ela e provada contra Postgres real; o estouro de cota que chega como HTTP 200 e nao abria o breaker; e a cota que era contada DUAS vezes. Leia antes de mexer em cadencia, ordem de candidato ou cota da OMDb.
...
- [`docs/operations/language-cutdown.md`](docs/operations/language-cutdown.md) — **o recorte de cinco idiomas (pt, en, es, ja, ko) e por que a coluna de idioma estava vazia**. A lista fechada era um GUARDA DE FK: `languages` tinha tres linhas e `movies.original_language` aponta para ela, entao todo idioma fora de `en`/`es` era descartado em silencio — 43% dos filmes e 60% das series com a coluna nula, e `pt`, `ja` e `ko` (tres dos cinco que FICAM) entre os nulos. Tem tambem: por que a ordem A->B->D e obrigatoria e o intertravamento que a torna executavel; a cascata REAL lida de `pg_constraint`, incluindo as 21 FKs para `entities` com ON DELETE RESTRICT que fazem um DELETE despreparado ABORTAR em vez de deixar orfa; por que o filtro de entrada nao cabe antes do detalhe (o export diario nao traz idioma) e custa zero requisicao onde esta. Leia antes de mexer em `original_language`, no gate de entrada do catalogo ou em apagamento em massa.
- [`docs/operations/text-recovery-measurement.md`](docs/operations/text-recovery-measurement.md) — **a sinopse e a biografia ja estavam no banco, e ninguem as lia**. O detalhe do TMDB vai com `language=pt-BR` e volta com `overview`/`biography` VAZIOS quando o titulo nao tem traducao naquele idioma — e o texto existe dentro do bloco `translations`, que todo `append_to_response` ja pedia. Tem tambem: o teto de 100.000 por tipo que fazia o censo reportar o proprio limite como total; por que preencher `people.biography` NAO tira ninguem de `no_biography` (a licenca e o gate); por que 2 de cada 3 pessoas nao tem slug; e **como conferir a recuperacao sem ser enganado pelo cache das fichas**. Leia antes de rodar `backfill-text` ou de acreditar num censo.
- [`docs/operations/admin-operacional.md`](docs/operations/admin-operacional.md) — **o painel operacional (`apps/admin`, raiz `/`) e o contrato de cada tela**: onde mora cada dado do catalogo e qual portao vale (imagem pela FONTE, trailer pela LINHA, nota pelo gate de leitura), a volta de cada fila (universo ÷ teto × ciclos/dia, vermelha acima de 30 dias), o que cada cota conta e por que a OMDb nao publica cabecalho, como o commit em execucao e medido pela IMPRESSAO DIGITAL do codigo (nunca por `CINERIE_BUILD_SHA`), o que existe e o que nao existe de usuario, e o que cada botao ENFILEIRA (`catalog_jobs` ou `scheduler_force_requests`, com auditoria na mesma transacao). O painel nao apaga, nao muda licenca nem cadencia e nao implanta servico. Leia antes de mexer em fila, heartbeat, `service_heartbeats`, `deploy_main_commits` ou nas acoes do admin.
- [`docs/operations/fila-represada-2026-09-16.md`](docs/operations/fila-represada-2026-09-16.md) — **por que detalhe, descoberta e `/changes` ficaram 14 dias parados sem erro nenhum**. O claim de `catalog_jobs` e uma fila GLOBAL por prioridade: 115.980 jobs/dia de midia POR EPISODIO na 80 ocupavam o worker, e os produtores (`discover_ids`, `sync_changes`) nasciam em 100, o default do schema. Tem tambem: o recorte `episodeMediaSeasons` (cascata automatica so nas temporadas recentes; pedido de pessoa em todas), a prioridade 20 dos produtores num builder unico, e por que `languages` tinha 3 linhas em producao com o codigo ja gravando 186 idiomas (o vocabulario so vinha pelo seed; o release nao roda seed). Leia antes de mexer em prioridade de job, na cascata de serie ou em `languages`.
- [`docs/operations/sigterm-e-pid1.md`](docs/operations/sigterm-e-pid1.md) — **por que nenhum desligamento gracioso rodava em producao**. O EasyPanel roda o "Comando" como `/bin/sh -c` NO LUGAR do ENTRYPOINT (o PID 1 do `screen-cron` nao tem `PWD` no ambiente): o dash fica no PID 1 sem handler, e o kernel descarta o SIGTERM do `docker stop`. Abaixo do `pnpm run`, um `sh -c` morria sem repassar; a CLI `tsx` mata o filho com SIGKILL se ele nao confirmar o sinal em 30 ms; e o `pnpm exec` (comando do painel do `cinerie-publication-worker`) morre ao repassar o sinal, entao o tini nao serve. Conserto: o proprio `/bin/sh` das imagens com comando no painel (`scripts/container/pid1-shell.sh`, um init em dash quando e o PID 1 com comando simples) e scripts `*:start` com `exec node --import tsx`. O mesmo `sh -c` ficava entre o pnpm (PID 1) e o Next dos tres apps: o `start` de `apps/*` e `exec next start` — o Next ja fecha o servidor e espera as requisicoes em curso sozinho, faltava o sinal chegar. Dentro do agendador, o elo da CLI `tsx`: `runScript` sobe as CLIs filhas com `node --import` (nunca a CLI `tsx`), e `sync-omdb-ratings` para ENTRE requisicoes no SIGTERM e grava a linha `aborted` com a cota que gastou — antes o lote cortado nao deixava linha e a cota sumia de `readSpentToday`. Leia antes de mudar o comando de um servico no painel (tem de ser SIMPLES), o `/bin/sh` da imagem, um script `*:start`, o `start` de um app ou o spawn de uma CLI filha.
